...
public class LifeCycleTestServlet extends HttpServlet {
	...
	public void init(ServletConfig config) {
		System.out.println("annotation 매핑 init() 메소드 호출됨..." + initCount++);
	}
	
	/* GET 방식의 요청이 오면 올 때마다 동작하는 메소드 */
	@Override
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		System.out.println("annotation 매핑 doGet() 메소드 호출됨..." + doGetCount++);
	}
	
	/* 서브릿 컨테이너가 종료될 때 호출되는 메소드 */
	@Override
	public void destroy() {
		System.out.println("annotation 매핑 destroy() 메소드 호출됨..." + destroyCount++);
	}
}
